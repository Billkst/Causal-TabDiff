...
def test_itransformer():
    try:
        ...
        wrapper = iTransformerWrapper(seq_len=3, enc_in=FEATURE_DIM)
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        ...
        forecast = wrapper.forecast(X_test, pred_len=6)
...
def test_timexer():
    try:
        ...
        wrapper = TimeXerWrapper(seq_len=3, enc_in=FEATURE_DIM)
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
...
if __name__ == '__main__':
    print("="*80)
    print("B2 Gate Test V8 - After Model-Specific Repairs")
    print("="*80)
    ...
    with open('outputs/b2_gate/final_gate_test_results_v8.json', 'w') as f:
        json.dump(RESULTS, f, indent=2)
    
    pass_count = sum(1 for r in RESULTS if r['final_status'] == 'PASS')
    print(f"\n{'='*80}")
    print(f"FINAL SUMMARY: {pass_count}/{len(RESULTS)} models PASS")
    print(f"Results saved to: outputs/b2_gate/final_gate_test_results_v8.json")
    print('='*80)
