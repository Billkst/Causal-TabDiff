...
if __name__ == '__main__':
    ...
    for i, config_params in enumerate(grid):
        ...
        try:
            ...
            table_all_win = True
            for bk, bval in BASELINE_V2_FRONTIER.items():
                if bk not in METRIC_DIRECTIONS: continue
                val = metrics[bk]
                direc = METRIC_DIRECTIONS[bk]
                
                if direc == 'higher' and val < bval:
                    table_all_win = False
                if direc == 'lower' and val > bval:
                    table_all_win = False
                    
            cfg_lines.append("")
            cfg_lines.append(f"**Gate Pass**: {gate_pass} (Hard Wins: {hard_wins}/5)")
            cfg_lines.append(f"**Dominates V2 Baselines**: {table_all_win}")
            
            report_lines.extend(cfg_lines)
            
            if (hard_wins > best_hard_wins) or (hard_wins == best_hard_wins and (not best_formal_gate_pass and gate_pass)):
                best_config = config_params['name']
                best_hard_wins = hard_wins
                best_formal_gate_pass = gate_pass
                best_table_all_win = table_all_win
                
            print(f"hard_wins={hard_wins}/5 table_all_win={table_all_win} gate={gate_pass} auc={metrics.get('TSTR_AUC',0):.6f} f1={metrics.get('TSTR_F1',0):.6f} ate={metrics.get('ATE_Bias',0):.6f}")
