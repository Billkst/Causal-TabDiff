...
def main() -> None:
    ...
    with torch.no_grad():
        model_score_low = wrapper.model.predict_outcome_proba(
            wrapper.model.sample_with_guidance(256, torch.zeros((256, 1), device=device), guidance_scale=2.0),
            torch.zeros((256, 1), device=device),
        )
        model_score_high = wrapper.model.predict_outcome_proba(
            wrapper.model.sample_with_guidance(256, torch.ones((256, 1), device=device), guidance_scale=2.0),
