...
class CausalTabDiffWrapper(BaselineWrapper):
    ...
    def _predict_outcome_from_semantic(self, x_semantic_last, alpha_target):
        ...
        if self.outcome_model is not None:
            score = self.outcome_model.predict_proba(features)[:, 1]
        else:
            score = 0.15 * alpha_np.reshape(-1) + 0.05 * np.sum(x_np, axis=1)
    ...
    def sample(self, batch_size, alpha_target, device):
        ...
        y_cf = self._predict_outcome_from_semantic(x_cf_semantic[:, -1, :], alpha_target)
