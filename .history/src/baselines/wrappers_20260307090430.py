...
class CausalTabDiffWrapper(BaselineWrapper):
    ...
    def fit(self, dataloader, epochs, device, debug_mode=False):
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(dataloader):
                ...
                loss = diff_loss + 0.5 * disc_loss + outcome_loss
                loss.backward()
                optimizer.step()
                ...
                if batch_idx % log_every == 0 or batch_idx == num_batches:
                    logger.info(
                        ...
                        f"loss={loss.item():.6f}, diff={diff_loss.item():.6f}, disc={disc_loss.item():.6f}, y={outcome_loss.item():.6f}"
                    )
    ...
    def sample(self, batch_size, alpha_target, device):
        ...
        combined_score = 0.5 * glue_score + 0.5 * model_score
