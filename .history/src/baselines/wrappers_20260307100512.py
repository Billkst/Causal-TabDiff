...
def _resolve_metadata_path():
    ...
    return os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', 'dataset_metadata.json'))
...
class TSDiffWrapper(BaselineWrapper):
    ...
    def fit(self, dataloader, epochs, device, debug_mode=False):
        ...
        try:
            from .tsdiff_core.model import TSDiffDDPM
        except Exception:
            import sys
...
class CausalTabDiffWrapper(BaselineWrapper):
    ...
    def __init__(self, t_steps, feature_dim, **kwargs):
        ...
        self.sample_model_score_weight = float(np.clip(kwargs.get('sample_model_score_weight', 0.5), 0.0, 1.0))
        self.sample_guidance_scale = float(max(0.0, kwargs.get('sample_guidance_scale', 2.0)))
            ...
            for i, batch in enumerate(dataloader):
                ...
                if batch_idx % log_every == 0 or batch_idx == num_batches:
                    logger.info(
                        ...
                        f"y={outcome_loss.item():.6f}, y_w={self.outcome_loss_weight:.2f}"
                    )
