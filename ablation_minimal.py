...
for val in configs[ablation_type]:
    ...
    for seed in [42, 52, 62, 72, 82]:
        ...
        train_loader = get_dataloader('data/landmark_tables/unified_person_landmark_table.pkl', 'train', 1024, seed)
        val_loader = get_dataloader('data/landmark_tables/unified_person_landmark_table.pkl', 'val', 1024, seed)
        ...
        best = 0.0
        for ep in tqdm(range(30), desc=f"{ablation_type}={val} seed={seed}"):
            model.train()
            for batch in train_loader:
                opt.zero_grad()
                out = model(batch['x'].to(device), batch['landmark'].float().to(device))
                loss = out['diff_loss'] + disc_w * out['disc_loss'] + traj_w * model.compute_trajectory_loss(out['trajectory'], batch['trajectory_target'].to(device), batch['trajectory_valid_mask'].to(device)) + F.binary_cross_entropy(out['risk_2year'], batch['y_2year'].to(device))
                loss.backward()
                opt.step()
            
            if (ep + 1) % 5 == 0 or ep == 29:
                model.eval()
                yt, yp = [], []
                with torch.no_grad():
