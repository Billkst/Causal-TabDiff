...
def test_wrapper(name, wrapper_class, train_loader, device):
    ...
    try:
        ...
        print(f"[1/3] 训练 (3 epochs)...")
        wrapper.fit(train_loader, epochs=3, device=device)
        print(f"✓ 训练完成")
...
def main():
    ...
    results['TabSyn'] = test_wrapper('TabSyn', TabSynLandmarkWrapper, train_loader, device)
    results['TabDiff'] = test_wrapper('TabDiff', TabDiffLandmarkWrapper, train_loader, device)
