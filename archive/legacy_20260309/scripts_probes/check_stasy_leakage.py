...
def train_tstr(fake_x, fake_y, real_x, real_y, seed=42):
    ...
    auc = roc_auc_score(real_y, prob)
    f1_default = f1_score(real_y, pred)

    ap = average_precision_score(real_y, prob)
    precision, recall, thresholds = precision_recall_curve(real_y, prob)
    f1_curve = (2 * precision * recall) / (precision + recall + 1e-12)
    best_idx = int(np.nanargmax(f1_curve))
    f1_best = float(f1_curve[best_idx])

    if best_idx == 0 or thresholds.size == 0:
        best_threshold = 0.5
    else:
        thr_idx = min(best_idx - 1, thresholds.size - 1)
        best_threshold = float(thresholds[thr_idx])

    return auc, f1_default, float(ap), f1_best, best_threshold
...
def quantile_map_fake_to_real(real_x, fake_x):
    real_mapped = real_x.copy()
    fake_mapped = fake_x.copy()
    n = fake_mapped.shape[0]
    if n == 0:
        return fake_mapped

    for j in range(fake_mapped.shape[1]):
        rcol = real_mapped[:, j]
        fcol = fake_mapped[:, j]

        order = np.argsort(fcol)
        target_q = np.quantile(rcol, (np.arange(n) + 0.5) / n)
        mapped = np.empty_like(fcol)
        mapped[order] = target_q
        fake_mapped[:, j] = mapped
...
def main():
    ...
    parser.add_argument("--calibrate_fake_y", action="store_true")
    parser.add_argument("--calibrate_fake_x", action="store_true")
    ...
    if args.calibrate_fake_y:
        target_rate = real_pos_rate
        k_pos = int(round(target_rate * len(fake_y)))
        k_pos = max(0, min(len(fake_y), k_pos))
        score = fake_y.astype(float)
        order = np.argsort(score)
        fake_y_cal = np.zeros_like(fake_y)
        if k_pos > 0:
            fake_y_cal[order[-k_pos:]] = 1
        fake_y = fake_y_cal
        fake_pos_rate = float(fake_y.mean())

    fake_x_raw = fake_x.copy()
    if args.calibrate_fake_x:
        fake_x = quantile_map_fake_to_real(real_x, fake_x)

    if len(np.unique(fake_y)) < 2 or len(np.unique(real_y)) < 2:
        tstr_auc, tstr_f1 = 0.5, 0.0
        tstr_auc_shuf, tstr_f1_shuf = 0.5, 0.0
        tstr_ap, tstr_f1_best, tstr_best_thr = 0.0, 0.0, 0.5
        tstr_ap_shuf, tstr_f1_best_shuf, tstr_best_thr_shuf = 0.0, 0.0, 0.5
    else:
        tstr_auc, tstr_f1, tstr_ap, tstr_f1_best, tstr_best_thr = train_tstr(fake_x, fake_y, real_x, real_y, seed=args.seed)
        shuffled = fake_y.copy()
        np.random.shuffle(shuffled)
        tstr_auc_shuf, tstr_f1_shuf, tstr_ap_shuf, tstr_f1_best_shuf, tstr_best_thr_shuf = train_tstr(fake_x, shuffled, real_x, real_y, seed=args.seed)

    d_auc = domain_auc(real_x, fake_x, seed=args.seed)
    mem_ratio, med_fr, med_rr = nn_memorization_ratio(real_x, fake_x_raw)
    ...
    print(f"TSTR ap={tstr_ap:.4f}, f1_best={tstr_f1_best:.4f}, best_thr={tstr_best_thr:.4f}")
    print(f"Shuffle-control TSTR auc={tstr_auc_shuf:.4f}, f1={tstr_f1_shuf:.4f}")
    print(f"Shuffle-control TSTR ap={tstr_ap_shuf:.4f}, f1_best={tstr_f1_best_shuf:.4f}, best_thr={tstr_best_thr_shuf:.4f}")
    ...
    if mem_ratio < 0.80:
        gate_fail_reasons.append(f"疑似记忆化(nn_mem_ratio={mem_ratio:.4f} < 0.8000)")
    ...
    report_lines = [
        ...
        f"- tstr_ap: {tstr_ap:.4f}",
        f"- tstr_f1_best: {tstr_f1_best:.4f}",
        f"- tstr_best_threshold: {tstr_best_thr:.4f}",
        f"- shuffle_tstr_auc: {tstr_auc_shuf:.4f}",
        f"- shuffle_tstr_f1: {tstr_f1_shuf:.4f}",
        f"- shuffle_tstr_ap: {tstr_ap_shuf:.4f}",
        f"- shuffle_tstr_f1_best: {tstr_f1_best_shuf:.4f}",
        f"- shuffle_tstr_best_threshold: {tstr_best_thr_shuf:.4f}",
        f"- domain_auc: {d_auc:.4f}",
        f"- nn_mem_ratio: {mem_ratio:.4f}",
        f"- calibrated_fake_y: {args.calibrate_fake_y}",
        f"- calibrated_fake_x: {args.calibrate_fake_x}",
        "",
