...
EPOCHS = 8
DIFFUSION_STEPS = 20
GUIDANCE_SCALE_WEIGHT = float(os.environ.get('CAUSAL_TABDIFF_SAMPLE_MODEL_SCORE_WEIGHT', '0.75'))
OUTCOME_LOSS_WEIGHT = 1.0
OUTCOME_RANK_LOSS_WEIGHT = float(os.environ.get('CAUSAL_TABDIFF_OUTCOME_RANK_LOSS_WEIGHT', '0.0'))
SAMPLE_GUIDANCE_SCALE = float(os.environ.get('CAUSAL_TABDIFF_SAMPLE_GUIDANCE_SCALE', '1.0'))
USE_TRAJECTORY_RISK_HEAD = os.environ.get('CAUSAL_TABDIFF_USE_TRAJECTORY_RISK_HEAD', '0') == '1'
RISK_SMOOTHNESS_WEIGHT = float(os.environ.get('CAUSAL_TABDIFF_RISK_SMOOTHNESS_WEIGHT', '0.0'))
CF_CONSISTENCY_WEIGHT = float(os.environ.get('CAUSAL_TABDIFF_CF_CONSISTENCY_WEIGHT', '0.0'))
SAMPLE_USE_TRAJECTORY_RISK = os.environ.get('CAUSAL_TABDIFF_SAMPLE_USE_TRAJECTORY_RISK', '0') == '1'
OUTPUT_PATH = os.environ.get(
    'CAUSAL_TABDIFF_REHEARSAL_OUTPUT_PATH',
    os.path.join(PROJECT_ROOT, 'logs', 'testing', 'causal_tabdiff_final_gated_rehearsal.md'),
)
...
def main() -> None:
    ...
    lines.append(f'- Use trajectory risk head: {USE_TRAJECTORY_RISK_HEAD}')
    ...
    lines.append(f'- Sample use trajectory risk: {SAMPLE_USE_TRAJECTORY_RISK}')
